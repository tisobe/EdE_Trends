...
#########################################################################################
#											#
#	ede_hrma.perl: extract and plot hrma temp - ede relationship			#
#											#
...
#
#--- only following msids have none constant temperature values.
#

@list = ('4rt555t','4rt556t','4rt557t','4rt558t','4rt559t','4rt560t','4rt561t','4rt562t','4rt563t','4rt564t','4rt565t','4rt567t','4rt568t','4rt569t','4rt570t','4rt575t','4rt576t','4rt577t','4rt578t','4rt579t','4rt580t','4rt581t');
...
while(<FH>){
        ...
        $tmonth= $atemp[6];
#
#----- change month in letter to numeric format
#
        ...
        $year  = $atemp[8];
#
#---- change month and date in the month to date from Jan 1
#
...
#
#--- check dataseeker only when there are new data
#
...
OUTER:
	for($i = 0; $i < $total; $i++){
#
#--- set data extracted interval for the first 5 min
#
		$date = $time[$i];
		chomp $date;
		@ttemp = split(/:/, $date);
...
#
#--- calling dataseeker: time interval is date<--->date2
#
		system("dataseeker.pl infile= print=yes outfile=outfile.fits search_crit=\"timestart=$date timestop=$date2  columns=mtatel..hrmatherm_avg\"");
			$chk = `ls ./`;
			if($chk !~ /outfile.fits/){
				next OUTER;
			}
#
#--- we got a file containing all hrma msids data for the time interval
#--- using dmlist, extract the first temperature value for the specific msid
#--- and record it	
#
		LOUTER:
		foreach $msid (@list){
...
#
#--- plotting start here. six figures in one page
#

$plt_cnt = 0;		# number of plotted on one page
...
#	
	...
	pgsci(1);
#
#--- we use only robust fit for this 
#
	@xdata = @xbin;
	@ydata = @ybin;
	$data_cnt = $dcnt;
...
#
#--- check whether we plotted six figures on one page
#--- if we did, go to a new page (tot_cnt)
#
	if($plt_cnt >= 5){
		pgclos();
		$plt_cnt = 0;
		...
		system("echo ''|gs -sDEVICE=ppmraw  -r256x256 -q -NOPAUSE -sOutputFile=-  ./pgplot.ps|$bi_dir/pnmcrop| $bi_dir/pnmflip -r270 |$bi_dir/ppmtogif > $out_plot");
		system("rm pgplot.ps");
	}else{
		$plt_cnt++;
	}
